...
params.num_iter = 500;
for i = 45 
mymodel = (['.\model\MWCNN_GDSigma5_3_3Dgrey_400_input_threewavelet\MWCNN_GDSigma5_3_3Dgrey_400_input_threewavelet-epoch-'  num2str(i,'%d') ]); 
load(mymodel);
net1 = net;
...
%% ÏÖÔÚµÄDAEP
[map_deblur_extend,psnr_ssim,psnr_psnr ]= MWDMSP_deblur(degraded, kernel, sigma_d, params, net1 ,aa);
...
save (['./debulur_sigma5/',fn1,'_simga',num2str(sigma,'%d'),'_simga',num2str(sigma_dae,'%d'),'_epoch',num2str(i,'%d'),'kernels',num2str(aa,'%d'),'_alpha',num2str(a1,'%d')]);
